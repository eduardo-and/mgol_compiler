...
class Scanner:
    ...
    reservedSymbolsList = [
        ...
        Token(tokenClass= TokenClass.facaate,type="faca-ate"),
    ...
    def scan(self)->Token:
        ...
        for line_index,line in enumerate(self.file):
            if(isStart):   
                if(line_index<self.currentLine):    
                    continue
                
            else:
                self.currentColumn=0
                self.currentLine=line_index
            for column_index, char in enumerate(line):
                if(isStart):   
                    if(column_index<self.currentColumn):    
                        continue
                isStart=False
                
                self.currentColumn=column_index
    ...
    def __initToken(self,collumn,line):
        return Token(
    ...
    def __find_token_by_lexeme(self,lexeme: str):
